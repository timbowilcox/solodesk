...
- **Loop 1 live verification (2026-05-08).** Two complete server-side runs against live Anthropic confirmed the Sprint 10 substrate behaves as designed (parser, runner, persistence, eventing, Watch narration, comment anchoring). Detail in `.archive/handoffs/loop-1-live-verification.md`. Two UI defects surfaced: (1) hydration mismatch on `/ventures/<slug>/strategy` cascading into Pause/Cancel never rendering; (2) no approve form for Loop-generated Documents (status='reviewing'). Both fixed in the **ui-fix sprint** the same day. Detail in `.archive/handoffs/ui-fix-handoff.md`.
- **Loop 1 production re-verification + cancel-fix sprint (2026-05-08).** With the ui-fix commits live on `app.solodesk.ai`, the original verification re-ran end-to-end against production. Confirmed live: hydration fix works, approve flow for `status='reviewing'` works (clicked through, decisions row written), Pause primitive works (server completed despite client pause). Two new substrate findings surfaced: (a) Cancel mid-stream produced `failed`/`drafting_orphaned` instead of the spec's `cancelled`/`cancelled` — root cause was the runner's catch block conflating client-abort-after-cancel with genuine Anthropic stream failure; (b) ConversationThread's second-submit silently failed because StreamingDocument's useEffect deps were `[streamRequest?.url]` (URL is constant). Both fixed in the **cancel-fix sprint**. Detail in `.archive/handoffs/cancel-fix-handoff.md`. Re-verification of these specific paths against the next deploy is the only remaining item before Loop 1 verification can be marked closed; the substrate questions are answered.
